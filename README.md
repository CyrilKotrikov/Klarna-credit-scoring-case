...
I decided to use both linear and non-linear models to approach this case. Logistic regression was selected as the main model because it provides strong scope for structured data preprocessing. In parallel, I used XGBoost as a non-linear benchmark model, as both research and my personal experience suggest that it often outperforms logistic regression in predictive tasks. (It is important to note that the objective of this case study is not to build the best possible model, but to demonstrate a structured modeling approach.)
